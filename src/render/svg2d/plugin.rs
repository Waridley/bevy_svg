...
use bevy::prelude::{Changed, Commands, Entity, EventReader, Mesh, Query, Res, ResMut, Last, IntoSystemConfigs, Vec3};
use bevy::render::mesh::PrimitiveTopology;
use bevy::render::render_asset::RenderAssetUsages;
use bevy::sprite::Mesh2dHandle;
use bevy::utils::HashMap;
use crate::plugin::Set;
use crate::render::{SvgMesh2d, SvgMesh3d, SvgMeshCache};
use crate::svg::Svg;
...
pub fn svg_mesh_2d_generator(
    ...
    for (id, settings, svg, existing_mesh) in &query {
        let mesh = cache.entry(svg.clone_weak())
        ...
        if let Some(svg) = svgs.get(svg) {
            *mesh = svg.tessellate(&settings.clone().into());
            debug!(?mesh);
        } else {
    ...
    for event in svg_events.read() {
        match event {
            ...
            AssetEvent::Modified { id } => {
                ...
                let cache = cache.entry(Handle::Weak(*id))
                  .or_insert_with(HashMap::default);
